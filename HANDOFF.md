...
- **🚀 v2 WENT LIVE 2026-06-11** (Jac approved; merged design-overhaul → main at 2d3d61e; CI boot check + Pages deploy green; live site verified serving the v2 code). Work continues on `design-overhaul` → merge to main per Jac's go-ahead each time.
- **Backend storage is SCHEMA-LESS** ([id, json] per row — see JacTec-handoff/Code.gs, now ON THIS MACHINE, gitignored, NEVER commit: it contains the role passwords and the repo is public). All new v2 fields persist with no backend change. **ONE pending backend step:** Jac must paste Code.gs v2 (adds `uploadCapture` → Drive) into the Apps Script editor, run `authorize` once (approve the new Drive permission), and redeploy as a New Version on the SAME deployment — until then capture VIDEOS toast an upload failure (stamps still persist fine).
...
## STREAMLINE CAMPAIGN (2026-06-11, IN PROGRESS — Jac's pre-debug consolidation)
...
- **Remaining (worklist in the audit JSON):** chrome batch (overlay popups: inspection/service/payment/addCard buttons → actionPill/ghostPill; winpicker Done/Clear/Today; pick-bar/guide Exit; board-view minis; login button; +New menu '+X' naming; site-suggestion buttons → dd-items) · Phase D aggressive dead-code (transportPicker chain + js-tnode/js-settransport/js-transport-sel handlers + .tnode/.tline/.sb-*/.gated/.boldlight/.tot-count/.filterbar/menu-block CSS + goBack + ringSVG + dashboardEl + unused 'fill' in DETAIL.rentals + js-field-call/js-wash-request handlers — evidence per item in the JSON) · section banners per the §-plan · **SPEC v7** (JacTec-handoff/, includes the R-rulebook).
