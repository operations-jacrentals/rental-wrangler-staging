...
- 🆕 **Tabbed message convos in the bottom tool bar** — DECISION (Jac): a tabbed conversation dock along the bottom tool bar, split by side:
  ...
  - 🆕 **INTERNAL side — enriched concept (Jac, 2026-06-14; photos incoming):** the internal chat is a **live feed of the Phase-6 record comments** ("what's flagged"), repliable. Plus: (a) **role buttons along the bottom of the window** — toggle a role to include/exclude them from the conversation; (b) **taggable elements** — pull records (units/customers/rentals/WOs/invoices…) INTO a chat like tags, add/remove them so **one thread persists while jumping task to task** (the conversation carries its own mutable context set). BUILD HELD pending Jac's reference photos; then spec via the `frontend` skill.
